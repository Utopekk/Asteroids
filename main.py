...
class AsteroidsGame:
            ...
            for asteroid_group in [self.vec_huge_asteroids, self.vec_medium_asteroids, self.vec_small_asteroids]:
                for asteroid in asteroid_group[:]:
                    ...
                    if enemy_rect.colliderect(asteroid_rect):
                        self.create_manager.create_particle_effect(self.enemy.x, self.enemy.y, WHITE, 5)
                        del self.enemy
                        self.enemy = self.create_manager.create_enemy()
    ...
    def check_player_collision_with_asteroids(self, player_rect):
        for asteroid in self.vec_small_asteroids[:]:
            ...
            if player_rect.colliderect(asteroid_rect):
                self.handle_manager.handle_player_asteroid_collision(asteroid)
            ...
            for asteroid_group in [self.vec_huge_asteroids, self.vec_medium_asteroids, self.vec_small_asteroids]:
                for asteroid in asteroid_group:
                    ...
                    if bullet_rect.colliderect(asteroid_rect):
                        if self.handle_manager.handle_bullet_asteroid_collision(asteroid):
    ...
    def check_player_enemy_collision(self):
        if self.game_over['status'] or self.player.destroyed or not self.enemy:
            return
    ...
    def run_game(self):
        ...
        while game_running:
            ...
            if not self.game_over['status']:
                ...
                font = pygame.font.Font(None, 48)
                stage_text = "Stage: " + str(self.level.stage)
